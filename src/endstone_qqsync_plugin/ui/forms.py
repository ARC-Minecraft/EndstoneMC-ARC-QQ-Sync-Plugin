        ...
        try:
            ...
            if hasattr(self.plugin, 'group_members') and self.plugin.group_members:
                if qq_input not in self.plugin.group_members:
                    player.send_message(f"{ColorFormat.GRAY}[QQsync] {ColorFormat.RED}该QQ号未加入QQ群，无法绑定！{ColorFormat.RESET}")
                    player.send_message(f"{ColorFormat.GRAY}[QQsync] {ColorFormat.YELLOW}请先加入QQ群后再试{ColorFormat.RESET}")
                    return
            ...
            if self.plugin.data_manager.bind_player_qq(player.name, player.xuid, qq_input):
                ...
                player.send_message(f"{ColorFormat.GRAY}[QQsync] {ColorFormat.AQUA}您的QQ {qq_input} 已与游戏账号绑定{ColorFormat.RESET}")
                ...
                player.send_message(f"{ColorFormat.GRAY}[QQsync] {ColorFormat.RED}绑定失败，请重试！{ColorFormat.RESET}")
